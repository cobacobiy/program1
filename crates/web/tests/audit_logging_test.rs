...
async fn setup_test_app() -> (axum::Router, String, String, Arc<AuditModule>, String, Uuid) {
    ...
    sqlx::query(
        "INSERT INTO buyer_accounts (id, google_sub, email, full_name, avatar_url, phone_number, phone_verified, is_active, created_at, updated_at)
         VALUES ($1, 'sub_audit', 'audit@buyer.com', 'Audit Buyer', NULL, '+628123456789', 1, 1, $2, $3)",
    )
    ...
    let buyer_dto = program1_contracts::BuyerAccountDto {
        id: buyer_id,
        google_sub: Some("sub_audit".to_string()),
        email: "audit@buyer.com".to_string(),
        full_name: "Audit Buyer".to_string(),
        avatar_url: None,
    ...
    sqlx::query(
        "INSERT INTO buyer_addresses (id, buyer_id, recipient_name, phone_number, street_address, subdistrict, city, province, postal_code, is_default, created_at, updated_at)
         VALUES ($1, $2, 'Audit Buyer', '+628123456789', 'Jl. Audit 1', 'Sub', 'City', 'Prov', '12345', 1, $3, $4)",
    )
...
async fn test_login_and_order_create_audit_logs() {
    let (app, admin_token, _, _, buyer_token, addr_id) = setup_test_app().await;
    ...
    let req = Request::builder()
        ...
        .header(header::CONTENT_TYPE, "application/json")
        .body(Body::from(
            serde_json::to_vec(&success_login_payload).unwrap(),
        ))
        .unwrap();
    let _ = app.clone().oneshot(req).await.unwrap();

    // 3. Place an order via buyer checkout
...
async fn test_non_admin_cannot_access_audit_logs() {
    let (app, _, staff_token, _, _, _) = setup_test_app().await;
