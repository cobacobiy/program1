...
// --- BUYERS DIRECTORY & CRM MODULE ---
let currentCustomers = [];
let currentCustomerTab = "list";
...
async function loadCustomersDirectory() {
  ...
  try {
    ...
    const verifiedCount = currentCustomers.filter(c => c.phone_verified).length;
    const activeCount = currentCustomers.filter(c => c.is_active).length;
      ...
      <tr>
        <td>
          <div style="display:flex; align-items:center; gap:0.6rem;">
            ...
            <div>
              <div style="font-weight:600; color:var(--text-main); font-size:0.85rem;">${b.full_name || "-"}</div>
              <div style="font-size:0.7rem; color:var(--text-muted); font-family:'JetBrains Mono';">${b.id.substring(0, 8)}...</div>
            </div>
          </div>
        </td>
        <td style="color:var(--text-main); font-size:0.85rem;">${b.email}</td>
        <td>
          <div style="display:flex; align-items:center; gap:0.4rem;">
            <span style="font-size:0.85rem;">${b.phone_number || "-"}</span>
    ...
    tbody.innerHTML = logs.map(l => {
      ...
      return `
        <tr>
          ...
          <td style="font-weight:600; font-size:0.85rem; color:var(--text-main);">${actorStr}</td>
          <td>
          ...
          <td style="font-size:0.8rem; color:var(--text-muted); max-width:320px; overflow:hidden; text-overflow:ellipsis; white-space:nowrap;">${detailsStr}</td>
        </tr>
      `;
    }).join("");
  } catch (err) {
